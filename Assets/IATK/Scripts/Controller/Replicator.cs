...
namespace IATK
{
    public class Replicator : MonoBehaviour
    {
        ...
        Dictionary<string, VisHolder> repliques = new Dictionary<string, VisHolder>();
        ...
        /// <param name="payload"></param>
        /// <returns></returns>
        public void UpdateRepliques(string uid, string payload)
        {
            Debug.Log("Replicator::UpdateRepliques uid=" + uid + ", #payload=" + payload.Length);
            if (uid.Length > 0)
            {
                if (repliques.ContainsKey(uid))
                {
                    var vis = repliques[uid].vis;
        ...
        private void SpawnReplicatedVis(string uid, string payload)
        {
            try
            {
                ...
                if (vis != null)
                {
                    ...
                    repliques.Add(uid, new VisHolder(vis));
